...
# min(country_population$year)
# #1960
# max(country_population$year)
# # 2022


country_population_final <- country_population %>% 
  filter(year >= 1990 & year <= 2010) %>%
  group_by(country, iso3c) %>%
  summarize(iso3c = iso3c,                                                                                            mean_population = mean(population)) %>% 
  distinct() %>%
  arrange(iso3c)

# !Obehi has other ways to use group by too!

country_population_final
```
...
```{r}
min(country_birth$year)
#1990
max(country_birth$year)
# 2015


country_birth_edits <- country_birth %>% filter(year >= 1990 & year <= 2010) %>% select(`iso3c`, `year`, `life_expect`)
country_birth_edits_2 <- country_birth_edits %>% filter(!is.na(iso3c) & !is.na(year) &!is.na(life_expect))

country_birth_final <- country_birth_edits_2 %>% 
  group_by(iso3c) %>% 
...
# rounds the 3 columns but then doesn't select the final
#country_birth_final  <- round(country_birth_final[, 2:4], 1)

country_birth_final 

```

# ! how to round without rounding each row individually?
...
There are 202 rows and 6 columns in the country_both tibble.

```{r}
country_both <- inner_join(country_population_final, country_birth_final, by ="iso3c" )
print(country_both, 10)
```
...
# we listed colors for the species alphabetical so  it attaches the right colors to the right penguins
...
penguins %>%
  ...
  #adds the linear regression line for the data frame and x, y above
  geom_smooth(method = lm, se = FALSE)
 

```

#! how to add label for alpha level?
...
The generate_random_graph function will sample 0 and 1 to fill an nxn matrix, with the probability of selecting 1 set at the density_low level of 0.1. Then the function takes the unsymmetric matrix and makes it symmetric by adding the transpose of the matrix. (When the function sees there is a cell with 2, it will replace it with 1. It also made the diagonol all 1s.)

Then it forms cliques and randomizes the nodes and computes the reverse order.  (!Idk what this means)
...
The Pivot_longer_heatmap() function takes the data from each of the cells in the adj_mat matrix, then makes a tibble and changes the names of the columns to "node:1" etc. and adds a column that states the Row number. 

Then it uses pivot_longer to creat a tibble that has 2 columns that states the nodes of the matrix, (leaving the Row column alone) and a third column that states the value of that cell. It then deletes the word "node:" from the column name, and renames the columns X and Y. Finally it returns the long format of the matrix with the name mat_long.
...
The heat map plot show which cells in the adj_mat matrix had 1 because they were colored coral2. Cells with 0 are colored palegoldenrod.
...
mat_long %>%
  ...
  scale_fill_manual(values =  c('0'= 'palegoldenrod', '1' = 'coral2'))+
    ylab("Row")+
    xlab("Column")+ 
  # changing legend from as.factor(Value)
    guides(fill=guide_legend(title="Value"))


# Fixed this error by using fill = as.factor(Value) instead of fill = Value
# Error in `scale_fill_manual()`:
# ! Continuous values supplied to discrete scale.
# ℹ Example values: 1, 0, 1, 0, and 0
...
The permuted heat map in adj_mat_2 shows us that there was a large clique in adj_mat. When that clique is permuted, it shows the connected nodes. 


```{r}
mat_long_2 <- pivot_longer_heatmap(adj_mat2)
...
To make sure the team's function is correct you can:
1. Check the private and public functions are labeled appropriately and work corrected.
2. You could try to permute it with the function in question 3. 
3. You can check the defaults of the function to make sure they are set appropriately.
4. You can make sure they follow the company's style guide and coding grammar. 
...
#Final Note: use rmarkdown::render("vignettes/HW2.Rmd") in console (not in Rmd)
