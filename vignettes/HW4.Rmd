...
My study group includes Gabby, Obehi, Max and Alejandro. I assume I will talk with them about this project. I don't plan to use ChatGPT, but my study group often does.  
Prof. Kevin helped me worked through the functions. I wound up using co-pilot for the unit tests. 
...
**Implementation 9** starts by checking the matrix meets the criteria from homework 3, then it excludes nodes of size 1 (that is the only size less than 2). It creates a helper function to compute the density of any clique then uses combinatorics to test all sub-matrices. It starts with the largest matrix and successfully found a partial clique. 

This function also checks that the adj_mat is actually a matrix, which I hope/assume my isSymmetric also checks. 

**Implementation 16** takes a similar approach using combinatorics to work down from the largest matrix testing the current edge density along the way. They describe this as a brute force approach (that will be limited efficiency with larger matrix) but it found the same clique as implementation 9 very quickly. 
...
**Implementation 9** 
No! They have thorough descriptions, and stop messages if there is a failure before the function runs. 
The implementation was slow when I tried to use the unit test for time but it passed 5 other tests. 

**Implementation 16** 
The Roxygen skeleton is sparse but it conveys the information. 
When I run the simulation study, I think it stalls on implementation 16, even though it is really similar to implementaiton 9. I don't know what the difference is. 
...
```{r, error = TRUE}
#Part 1
context("Testing compute_maximal_partial_clique9")

#1
test_that("compute_maximal_partial_clique9 works", {
  set.seed(11)

  res1 <- generate_partial_clique(n = 10,
                                 clique_fraction = 0.5,
                                 clique_edge_density = 0.9)
  res2 <- compute_maximal_partial_clique9(adj_mat = res1$adj_mat,
                                        alpha = 0.5)
...
#2
test_that("compute_maximal_partial_clique9 give an actual edge of a clique", {
  library(matrixcalc)
  set.seed(10)
...
# 3
test_that("compute_maximal_partial_clique9 rejects empty matrix", {
  set.seed(10)

  res3 <- as.matrix(0)
  expect_error(compute_maximal_partial_clique9(adj_mat = res3,
                                              alpha = 1))
})
#passed!

#4
test_that("compute_maximal_partial_clique9 won't work with permuted rows", {
  set.seed(10)
  ...
  expect_error(compute_maximal_partial_clique9(adj_mat = res7,
                                              alpha = 1))
})
# passed!
...
#5
test_that("compute_maximal_partial_clique9 rejects rectangle matrix", {
  set.seed(10)

  res1 <- as.matrix(c(1,2,3,4,5,6))

  expect_error(compute_maximal_partial_clique9(adj_mat = res1,
                                              alpha = 1))
})
#passed!
...
#6
# test_that("compute_maximal_partial_clique9 runs in less than 30 seconds", {
#   start_time <- Sys.time()
#   res1 <- generate_partial_clique(n = 30,
#                                   clique_fraction = 0.5,
#                                   clique_edge_density = 0.9)
#   res2 <- compute_maximal_partial_clique9(adj_mat = res1$adj_mat,
#                                          alpha = 1)
...
# })
#stalled for a long time but ultimately said passed!
...
```{r, error = TRUE}
#Part 2 
context("Testing compute_maximal_partial_clique16")

#1
test_that("compute_maximal_partial_clique16 works", {
  set.seed(12)

  res1 <- generate_partial_clique(n = 10,
                                 clique_fraction = 0.5,
                                 clique_edge_density = 0.9)
  res2 <- compute_maximal_partial_clique16(adj_mat = res1$adj_mat,
                                        alpha = 0.5)
...
#2
test_that("compute_maximal_partial_clique16 give an actual edge of a clique", {
  library(matrixcalc)
  set.seed(10)
...
# 3
test_that("compute_maximal_partial_clique16 rejects empty matrix", {
  set.seed(10)

  res3 <- as.matrix(0)
  expect_error(compute_maximal_partial_clique16(adj_mat = res3,
                                              alpha = 1))
})
#passed!

#4
test_that("compute_maximal_partial_clique16 won't work with permuted rows", {
  set.seed(10)
  ...
  expect_error(compute_maximal_partial_clique16(adj_mat = res7,
                                              alpha = 1))
})
# passed!
...
#5
test_that("compute_maximal_partial_clique16 rejects rectangle matrix", {
  set.seed(10)

  res1 <- as.matrix(c(1,2,3,4,5,6))

  expect_error(compute_maximal_partial_clique16(adj_mat = res1,
                                              alpha = 1))
})
#passed!
...
#6
# test_that("compute_maximal_partial_clique16 runs in less than 30 seconds", {
#   start_time <- Sys.time()
#   res1 <- generate_partial_clique(n = 30,
#                                   clique_fraction = 0.5,
#                                   clique_edge_density = 0.9)
...
I will use generate_partial_clique with 

n = 10
clique fraction = 0.5
and 2 levels of clique edge density = 0.9
...
We will vary 2 levels of clique edge density (alpha = 0.5 and 0.9.)
...
I will be using all 25 implementations. 

- 4) What do you hope to learn with your simulation study? Alternatively, what trends/patterns/results are you hoping to see?

Which implementations consistently do better at finding a large (or the largest) partial clique.

- 5) How many trials do you intend to run per "level"? 

5 trials per level of alpha

- 6) How much time do you estimate your simulation study will need at maximum? See Note 2 below.

5 trials per alpha* 2 levels of alpha* 25 implementations * 30 seconds maximum (even for n= 10 and clique fraction = 0.5 )
5 * 2 * 25 * 30 =7500 seconds or  125 minutes (about 2 hours 5 minutes max)
...
```{r}
#knitr::include_graphics("devtools HW4_1.png")

```

```{r}
#knitr::include_graphics("devtools HW4_2.png")
...
#knitr::include_graphics("~/UWBiost561/UWBiost561/vignettes/hw4-demo_bayes_plot.png")
```
...
I will do the basic final project you outlined. 
...
The instructions above are unclear when we should connect to Bayes. From 3A I think you want us to upload to github then pull to Bayes, before 3B says to connect to Bayes


Finally, I had a very hard time connecting to Bayes without getting kicked off, timed out, or unable to open Open Sesame. I can easily run this simulation on my laptop and create the image. But I don't feel confident I could do this on Bayes. 

#Final Note: use rmarkdown::render("vignettes/HW4.Rmd") in console (not in Rmd)
