...
ggplot_list <- lapply(1:length(level_trial_list), function(level_number){
  ...
  #changed lines here don't change for Bayes
    print(trial_mat)
trial_mat= matrix(trial_mat, ncol=2)
print(trial_mat)
#changed lines to there don't change for Bayes

tabulate_vec <- rowSums(trial_mat)

#changed lines here don't change for Bayes
  method_names <- as.character(1) #1:25
...
#changed lines here with file path don't change for Bayes
ggplot2::ggsave(plot_all, file = "hw4-demo_bayes_plot.png",
